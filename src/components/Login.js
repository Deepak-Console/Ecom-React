...
const Login = () => {
    ...
    const loginHandler = async () => {
        ...
        try {
            ...
            document.cookie = `token=${token};max-age=3600;path=/`; // Store token in cookies
            dispatch(login({ name, phoneNumber })); // Dispatch login action
            const redirectTo = location.state?.from || '/profile'; // Redirect to profile or another page
            navigate(redirectTo); // Navigate to the next page
        } catch (error) {
